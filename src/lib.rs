...
use iced::window;
use iced::{
    Alignment,  Element, Length, Point, Rectangle, Renderer,
    Size, Theme, Vector,
};
...
for Overlay<'a, 'b, Message>
{
    ...
    fn on_event(
        ...
        if let Event::Window(.., window::Event::RedrawRequested(now)) = &event{
            // window::Event::RedrawRequested(now), ..) = &event {
...
fn primary(theme: &Theme) -> container::Appearance {
    let palette = theme.extended_palette();

    styled(palette.primary.weak)
}

fn secondary(theme: &Theme) -> container::Appearance {
    let palette = theme.extended_palette();

    styled(palette.secondary.weak)
}

fn success(theme: &Theme) -> container::Appearance {
    let palette = theme.extended_palette();

    styled(palette.success.weak)
}

fn danger(theme: &Theme) -> container::Appearance {
    let palette = theme.extended_palette();
